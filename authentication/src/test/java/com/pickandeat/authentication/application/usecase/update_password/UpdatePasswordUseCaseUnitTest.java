...
public class UpdatePasswordUseCaseUnitTest {
    ...
    public void setUp() {
        ...
        this.updatePasswordUseCase = new UpdatePasswordUseCase(this.credentialsRepository, this.passwordService);
    }

    @Test
    public void updatePassword_shouldThrowException_whenUserIdNotFound() {
        when(this.credentialsRepository.findByUserId(command.userId().toString())).thenReturn(Optional.empty());
    ...
    public void updatePassword_shouldThrowException_whenOldPasswordNotMatch() {
        ...
        when(this.credentialsRepository.findByUserId(command.userId().toString())).thenReturn(Optional.of(credentials));
        when(this.passwordService.matches(command.oldPassword(), credentials.getPassword())).thenReturn(false);
        assertThrows(PasswordNotMatchException.class, () -> this.updatePasswordUseCase.execute(command));
    }
    ...
    public void updatePassword_shouldThrowException_whenPasswordEncryptionFailed() {
        ...
        when(this.credentialsRepository.findByUserId(command.userId().toString())).thenReturn(Optional.of(credentials));
        when(this.passwordService.matches(command.oldPassword(), credentials.getPassword())).thenReturn(true);

        when(this.passwordService.hashPassword(command.newPassword())).thenThrow(rootCause);
        assertThrows(CannotHashPasswordException.class, () -> this.updatePasswordUseCase.execute(command));
    }
    ...
    public void updatePassword_shouldThrowException_whenUpdateNotWorking() {
        ...
        assertThrows(DatabaseTechnicalException.class, () -> this.updatePasswordUseCase.execute(command));
    }
