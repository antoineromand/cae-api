package com.clickandeat.account.infrastructure.database;

import com.clickandeat.account.TestConfiguration;
...
public abstract class AbstractDatabaseContainersTest {
    ...
    private static void configurePostgreSQLDatabase(DynamicPropertyRegistry registry) {
        if (SharedPostgresContainer.isEnabled()) {
            ...
            System.out.println(Flyway.configure().
                    dataSource(container.getJdbcUrl(), container.getUsername(), container.getPassword())
                    .load().getConfiguration().getTable());
        } else {
